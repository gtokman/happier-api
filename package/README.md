...
```bash
npm install happier-api
# or: bun add happier-api
```

Ships compiled ESM with type declarations — works on Node 18+, Bun, and Deno.
...
the resulting Firebase ID token to Happier as `x-vendora-authentication` (raw,
no `Bearer` prefix). That's why a capture filtered to `ecom-api.getjaldi.com`
shows only the token, never the login.
...
Two details are load-bearing and easy to get wrong — both handled for you:
...
Everything (key, tenant, bundle id, app id) is preconfigured — nothing to supply:
...
Override the key, tenant, bundle id or app id per call to point at a different
Firebase project; `HAPPIER_FIREBASE_API_KEY` in the environment overrides the
built-in key.
...
An OpenAPI 3.1 spec for the same surface ships with the package at
`happier-api/openapi.yaml` — parameterized paths, a documented `vendoraAuth`
security scheme, no captured tokens. Feed it to `openapi-typescript`, Swagger
UI, Postman, or a generator in another language.

```ts
import spec from "happier-api/openapi.yaml"; // with a YAML loader / bundler
```

## Errors

`HappierApiError` (non-2xx, with `.status`/`.body`), `HappierAuthError`
(`.code` is Firebase's string, e.g. `INVALID_PASSWORD`,
